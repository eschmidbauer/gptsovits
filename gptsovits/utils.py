...
def clean_checkpoints(path_to_models="logs/44k/", n_ckpts_to_keep=2, sort_by_time=True):
    ...
    def name_key(_f): return int(re.compile("._(\d+)\.pth").match(_f).group(1))
    def time_key(_f): return os.path.getmtime(os.path.join(path_to_models, _f))
    ...
    def del_info(fn): return logger.info(f".. Free up space by deleting ckpt {fn}")
