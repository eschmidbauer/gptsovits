...
from gptsovits.feature_extractor.cnhubert import CNHubert
from gptsovits.module.mel_processing import spectrogram_torch
from gptsovits.module.models import SynthesizerTrn
from gptsovits.text_segmentation_method import splits
from gptsovits.TextPreprocessor import TextPreprocessor
from gptsovits.tools.i18n.i18n import I18nAuto, scan_language_list
from gptsovits.tools.mutils import load_audio
...
default_v2:
  ...
  vits_config_path: configs/s2_config.json
...
class TTS_Config:
    ...
    def __init__(self, configs: Union[dict, str] = None):
        ...
        self.vits_config_path = self.configs.get("vits_config_path", None)
        self.vits_weights_path = self.configs.get("vits_weights_path", None)
        self.bert_base_path = self.configs.get("bert_base_path", None)
        self.cnhuhbert_base_path = self.configs.get("cnhuhbert_base_path", None)
        self.languages = self.v2_languages

        if (self.t2s_weights_path in [None, ""]) or (not os.path.exists(self.t2s_weights_path)):
            self.t2s_weights_path = self.default_configs[default_config_key]['t2s_weights_path']
            print(f"fall back to default t2s_weights_path: {self.t2s_weights_path}")
        if (self.vits_config_path in [None, ""]) or (not os.path.exists(self.vits_config_path)):
            self.vits_config_path = self.default_configs[default_config_key]['vits_config_path']
    ...
    def update_configs(self):
        self.config = {
            ...
            "vits_config_path": self.vits_config_path,
...
class TTS:
    ...
    def _init_models(self,):
        ...
        self.init_vits_weights(self.configs.vits_config_path, self.configs.vits_weights_path)
    ...
    def init_vits_weights(self, config_path: str, weights_path: str):
        print(f"Loading VITS weights from {config_path} {weights_path}")
        self.configs.vits_config_path = config_path
        ...
        print(vits_model.load_state_dict(torch.load(weights_path, map_location=self.configs.device)["weight"], strict=False))
    ...
    def audio_postprocess(self, audio: List[torch.Tensor], sr: int, batch_index_list: list = None, speed_factor: float = 1.0, split_bucket: bool = True, fragment_interval: float = 0.3) -> Tuple[int, np.ndarray]:
